...
- Your final message must be the COMPLETE report in the mandated format, and
  nothing else. It will be filed verbatim as dev-docs/GwzRemoteTransportInterfaces-ReviewCode.md — write it as a
...
REVIEW AREAS
Attack the full schema/runtime API freeze using prior accepted checkpoints as evidence, with changed-range focus on stream io_clock/machine/async APIs, formatter pin/CI declaration, consumer admission and paired typed/encoded message fixtures. Test actual signatures/lifecycle, clock budget math, admission before effects, retained nominal schema/runtime identity, finite ingress/queue/control duties and unsupported capabilities. Verify all freeze evidence claims and deferrals are honest. No physical carrier should enter the implementation.

COMMANDS
Working directory /Users/owebeeone/limbo/gwz-dev.
Inspection: git show/diff/rev-parse/status, rg, read files at pinned revisions. Allowed focused gates from workspace root: cargo +1.95.0 test --manifest-path gwz-transport/Cargo.toml --locked --test io_clock --test io_clock_async --test pool_io_clock; explicit regeneration and archive proof commands in the checkpoint. Use targeted tests only to resolve concrete uncertainty; prior full-suite results are recorded evidence. No compilation outside these commands and no mutation. No large campaigns.
...
Dispatch command addendum: also permitted from the workspace root: `cargo +1.95.0 test --manifest-path gwz-transport/Cargo.toml --locked --test network_timeouts --test policy`.
