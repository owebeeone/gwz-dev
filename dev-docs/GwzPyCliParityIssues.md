...
| `repo` | Nested `add`, `create`, `sync`, `clone`, `detach`, and `attach`; add/create/clone expose matching `--member-id` / `--source-id` flags. | Same nested verbs and identity flags; clone has direct + stream clients, detach/attach have direct clients, and selector conflicts match Rust. | Lifecycle parameter/dispatch parity is fixed in the current source tree and covered by shared parser fixtures plus native/direct/submit tests. Generic cross-command JSON-shape parity remains tracked separately. |
