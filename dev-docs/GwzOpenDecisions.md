...
| D9 | `gwz local dispose --force` reports the waivers the operator passed (`forced past: dirty, unpreserved-history`), not the hazards actually raised and waived (probe note F6). Report what it waived, which needs the delete path to carry the raised hazards into `DeleteReport`. | gwz-core | the next gwz-core lane that touches disposal (clean-up Phase 2) |
