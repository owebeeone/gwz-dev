...
- Your final message must be the COMPLETE report in the mandated format, and
  nothing else. It will be filed verbatim as dev-docs/GwzRemoteTransportPlacementC-ReviewCode.md — write it as a
...
REVIEW AREAS
Review the in-process embedding proof and its scope claims: src/transport_host/message_embedding_tests.rs, tests/transport_backend/python_embedding.{rs,py}, preparation/tests/README and PlacementC.md. Establish actual CLI/core request-type identity from gwz-cli source, actual gwz-py codec invocation, candidate schema selection, exact message/byte preservation and attachments routed before ordinary dispatch. Review public manifest/schema/API isolation and bounded fixture ownership. This proves embedding at current message boundaries, not full executable or production native bridge activation.
