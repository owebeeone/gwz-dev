...
Status: recommendation review only. Do not implement this plan until the current
feature wave lands.
...
This document records the recommended GWZ documentation work after the next
feature set stabilizes. It is intentionally a plan, not an implementation. The
actual documentation should wait for the in-flight tag redefinition, branch
support, stash support, and any adjacent command-surface changes to land.
...
- The command list is necessarily unstable while tag, branch, stash, and related
  workflow changes are still in flight.
...
- `gwz-core/docs/Reference.md` is a useful start, but it is incomplete relative
  to the current protocol and code. For example, the current protocol includes
  `CaptureRequest`, `CommitRequest`, and `StageRequest`, which are not covered
...
- `gwz-core/dev-docs/GWZDesign.md` is marked authoritative, but some older text
  ...
  `gwz.conf/gwz.yml`, `gwz.conf/gwz.lock.yml`, `gwz.conf/snapshots`, and
  `gwz.conf/tags`; older docs mention names such as `workspace.gwz.yaml` and
  `.gwz/snapshots`.
- `gwz-core/dev-docs/GWZTagPlan.md` redefines `gwz tag` from a GWZ workspace tag
  artifact into real Git tag management. User-facing docs should not be finalized
  until that lands.
...
- Agent cautions:
  ...
  - do not finalize docs for tag, branch, or stash until their implementations
...
- Root workspace workflow:
  ...
  - snapshot/materialize/pull/push;
...
- `gwz-cli/docs/QuickStart.md`: create or clone a workspace, inspect status,
  add/create members, stage, commit, snapshot, materialize, pull, and push.
- `gwz-cli/docs/Concepts.md`: workspace root, member repo, manifest, lock,
  snapshot, Git tag, branch, stash bundle, selection, dry-run, partial, force,
  remotes, progress events.
...
- `gwz-cli/docs/MachineOutput.md`: `--json`, `--jsonl`, status porcelain,
  response envelope shape, event stream shape, exit-code expectations.
- `gwz-cli/docs/RootWorkspace.md`: using `gwz` to manage the development root
  workspace and other GWZ-managed roots.
...
- `gwz-core/docs/WorkspaceArtifacts.md`: current manifest, lock, snapshot,
  tag/stash metadata, local runtime state, schema versions, atomic write rules,
  and artifact examples.
- `gwz-core/docs/GitBackend.md`: `GitBackend` boundary, Git2 backend behavior,
  credentials, transfer progress, fallback rules.
...
These packages are designed so many agents can work independently after the
feature wave lands. Each package should keep edits within its listed ownership
area unless a dependency explicitly calls for cross-links.

| ID | Package | Depends On | Primary Outputs | Acceptance Criteria |
| --- | --- | --- | --- | --- |
| D0 | Source-of-truth freeze | Tag, branch, stash, and adjacent command changes landed | Updated inventory notes in this file or a short successor note | Final command list, artifact paths, protocol schema, and feature status are confirmed against code and tests. |
| D1 | Root agent guide | D0 | `AGENTS_GWZ.md`, optional root `AGENTS.md` pointer | Agents can install or run `gwz`, inspect the root workspace, stage/commit through GWZ, and avoid stale tag/branch/stash guidance. |
...
| C2 | CLI reference generator | D0, C1 | generator script or task, `gwz-cli/docs/CLI.md` | Generated reference matches `gwz --help` and command-level help; drift can be detected. |
| C3 | CLI command pages | D0, C1 | `gwz-cli/docs/commands/*.md` | Every stable command family has synopsis, mutation semantics, flags, examples, output, and failure notes. This can be split one agent per command family. |
| C4 | CLI workflow docs | C1, C3 | `QuickStart.md`, `Workflows.md`, `RootWorkspace.md` | Workflows are executable against the final CLI and do not rely on deprecated tag artifacts. |
| C5 | Machine output docs | D0, C1 | `MachineOutput.md` | JSON, JSONL, status porcelain, response envelopes, event streams, and exit behavior are documented with checked examples. |
...
| K3 | Workspace artifact docs | D0, K1 | `WorkspaceArtifacts.md` | Current artifact paths, schemas, examples, atomicity rules, and generated/local boundaries match code. |
| K4 | Git backend docs | D0, K1 | `GitBackend.md` | Backend responsibilities, credential behavior, transfer progress, and Git command fallback rules are documented. |
| P1 | Protocol catalog generator | D0 | catalog generator, `MessageCatalog.md` | Catalog is generated from `protocol/gwz.taut.py`, includes methods/enums/messages/fields/tags, and has a drift check. |
...
| V2 | Link and drift checks | all docs packages | CI or local verification commands | Local links pass, generated docs are current, and docs do not mention removed tag artifacts as live behavior. |
...
1. Wait for tag, branch, stash, and any other command-surface changes to land.
...
- Avoid documenting planned tag, branch, or stash behavior as available until
  the corresponding feature is merged and verified.
...
- Should old `GWZDesign.md` artifact-layout sections be updated directly after
  the feature wave, or should user-facing docs simply supersede them and leave
