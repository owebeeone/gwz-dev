...
- Your final message must be the COMPLETE report in the mandated format, and
  nothing else. It will be filed verbatim as dev-docs/GwzRemoteTransportPlacementDesign-ReviewSurface-1.md — write it as a
...
Read only gwz-core/docs/TransportPlacement.md and docs/Embedding.md, and existing CLI help if needed. NO design/plan/code. This is a proposed embedding API; no new CLI command is authorized, so perform the lifecycle walkthrough from the guide rather than inventing CLI syntax. Do not relitigate carrier ownership or local/cli choice; names, defaults, discoverability and lifecycle shape remain in scope.
...
Read only revised docs/TransportPlacement.md and docs/Embedding.md, your own prior Surface report and help if needed. Do not read design, source, remediation plan or other reports. Proposed callable names and usage now specified; trace example against declared signatures as design closure; actual compilation is a later implementation gate.
