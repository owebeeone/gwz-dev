...
*(The M4 scenario map that stood here was deleted at the M5d landing,
2026-09-05, by operator ruling. It mapped v0 durable progress shapes to the
tests that pinned them; M5d deleted the v0 engine and those suites, so its
subject no longer exists. It was also name-coupled and broke on rename. The
g23 partition count in `run_r4bg_aggregate_gates.py` guards that suite's
size instead.)*
