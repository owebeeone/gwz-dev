...
| F-5 | MAX_PATH / private-area relocation candidate (§1.2 item 7) | **MISSING (evidence) + open product decision** | Real product exposure: staged `ca1-*.source` names ≈173 chars breach MAX_PATH on default Windows; currently *test-masked* by the run-7 `core.longpaths=true` fixture pin (ledger :158-161), so CI is green while an ordinary user without the registry/git opt-in is not covered. Interim evidence action G-5 (fail-closed negative test without longpaths); the relocation itself ("under `.git/`") is a product package needing design + dual review — rescope-class routing in §5.3. |
