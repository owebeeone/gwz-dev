...
| G2 | Decision 1 **B** landed *(restated 2026-08-25 per R2.3 G2-c4; the original "filters-off at every materialization edge" phrasing was wrong — it would break the porcelain parity the amendment deliberately preserves)*: repo-local birth-time `core.autocrlf=false` + `core.eol=lf` pins at BOTH gwz repository-creation edges (`create_repo`; the single production clone funnel), the funnel's initial checkout additionally filters-off to cover the one window preceding its pin; every later worktree-writing edge in a gwz-born repo — checkouts, resets, fast-forwards, merge materialization, `stash_save`/`stash_apply` internal resets — is neutralized by the repo-local pins, with the two recovery-grade edges filters-off under Clause A; plus the CRLF sentinel riding every lane | Packet §1.5: "Option B in the next release train … every release shipped without the pins enlarges the legacy-workspace population" | R2 |
...
| G5 | Native evidence at the release candidate: three-arm matrix green at the RC sha; retained-readers green with the new generation *(corrected 2026-08-25 at the RC record: the original row also named the T-5 pair regeneration — over-eager; v0.11.0 regenerates no retained-reader evidence, so T-5 stays on its R2-F §11.2 carrier and is not a gate here)* | R2-F/R5 standing release gates (must-not-flip item 7 kept them unmoved for exactly this moment) | R5 |
